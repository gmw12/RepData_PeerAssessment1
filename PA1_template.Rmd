...
The mean total number of steps per day is `r mean_steps` and the median is `r median_steps`.
...
The `r max_interval` 5 minute interval has the maximum number of steps (`r max_steps`)
...
#impute NA values with the mean of the interval
...
After imputing NA values (#`r total_na`) with the mean of the interval: the mean total number of steps per day is `r mean_steps_imputed` and the median is `r median_steps_imputed`.
...
It appears that activity starts later on the weekend!
